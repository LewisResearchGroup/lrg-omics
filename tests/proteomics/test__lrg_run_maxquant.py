...
def test__missing_faa_raises_exception(tmpdir):
    ...
    cmd = f'lrg_run_maxquant.py --fasta {fn_faa} --raw {fn_raw} --mqpar {fn_mqp} --run-dir {run_dir} --out-dir {out_dir}  --maxquantcmd maxquant --verbose --add-raw-name-to-outdir --add-uuid-to-rundir'
