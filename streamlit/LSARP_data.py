...
from lrg_omics.LSARP.tools import (
    get_shipments,
    get_broad_plates,
    get_proteomics_plates,
    get_metabolomics_plates,
    get_growth_protein,
    get_growth_monitor,
)
...
show = st.sidebar.selectbox(
    "Show", ["APL Shipments", "Broad", "Metabolomics", "Proteomics", "Growth"]
)
...
if show == "APL Shipments":
        ...
        for plate in plates:
            ...
            if show_element == "Layout":
                crosstab = pd.crosstab(
                    _plate.PLATE_COL, _plate.PLATE_ROW, _plate.ISOLATE_NBR, aggfunc=sum
                )
    ...
    if show_element == "Statistics":
        ...
        chart = alt.Chart(_data).mark_bar().encode(y="# Isolates", x="DATE shipped")
        ...
        chart = alt.Chart(_data).mark_bar().encode(x="ORGANISM", y="Counts")
    ...
    if plates is not None:
        ...
        for plate in plates:
            ...
            if _broad_id_counts.max() > 1:
                st.write(
                    _broad_id_counts[_broad_id_counts > 1]
                    .to_frame()
                    .style.highlight_max(axis=0, color="red")
                )
            st.markdown("---")
...
if show == "Metabolomics":
    ...
    if show_element == "Layout":
        ...
        for plate in plates:
            st.header(plate)
            tmp = (
                grps.get_group(plate)[["PLATE_ROW", "PLATE_COL", "BI_NBR"]]
                .fillna("")
                .drop_duplicates()
            )
            crosstab = pd.crosstab(
                tmp["PLATE_ROW"], tmp["PLATE_COL"], tmp["BI_NBR"], aggfunc=sum
            )
            st.dataframe(crosstab)
            st.markdown("---")
...
if show == "Growth":
    ...
    if plates is not None:
        ...
        for plate in plates:
            ...
            if show_element == "Layout":
                crosstab = pd.crosstab(
                    _data["PLATE_ROW"], _data["PLATE_COL"], _data["OD"], aggfunc=np.mean
                )
                cm = sns.light_palette("green", as_cmap=True)
                st.dataframe(
                    crosstab.style.background_gradient(cmap=cm, low=od_min, high=od_max)
                )
