...
def update_rawtools_qc_data(raw_root, output_root=None, run=False, verbose=False):
    ...
    '''
    commands = []
    if verbose: print('Updating rawtools QC data:')
    for raw_file in get_all_raws(raw_root):
        if verbose: print(f' {raw_file}')
        cmds = rawtools_cmds(raw_file, 
                             raw_root=raw_root, 
...
def rawtools_cmds(raw, raw_root, output_root=None, 
    ...
    if ((isdir(output_dir) 
         and rawtools_output_files_exist(output_dir)) 
        and not force):
    ...
    if verbose:
        for cmd in commands:
            print(f' CMD: {cmd}')
    if run:
        if verbose: print('Running: ', output_dir)
        for cmd in commands:
