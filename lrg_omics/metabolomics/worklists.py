...
def read_worklist(fn):
    ...
    df["PLATE_COL"] = df.Position.apply(
        lambda x: "{:02.0f}".format(int(x.split(":")[1][1:]))
    )
    if not df.MS_FILE.duplicated().sum() == 0:
        df[df.MS_FILE.isin(df[df.MS_FILE.duplicated()].MS_FILE)].sort_values("MS_FILE")
...
def get_metadata_from_worklist(worklist, parse_func="LSARP"):
    ...
    metadata = pd.merge(worklist, metadata, on="MS_FILE", how="right").sort_values(
        ["PLATE_ID", "PLATE_ROW", "PLATE_COL", "RPT"]
    )
    metadata = metadata[
        [
            "MS_FILE",
