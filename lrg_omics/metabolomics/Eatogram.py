...
class Eatogram:
    ...
    def __init__(
        ...
        name,
        low_values_mask=1e4,
        media_type_name="MH-Pool",
        batch_col_name="PLATE",
        type_col_name="Type",
    ):
        """
        Parameters
    ...
    def calculate_mh_scores(self):
        ...
        mh_mean = (
            media_type_data.groupby(["peak_label", self.batch_col_name], dropna=False)
            .peak_max.mean()
            .to_frame()
            .add_suffix("_mh_mean")
            .reset_index()
        ...
        X = (
            self.df1.groupby(["ms_file", "peak_label"])
            .mean()
            .peak_max_mh_score.unstack("peak_label")
        )
    ...
    def finalize_data(self):
        ...
        self.df2 = pd.merge(
            self.df1,
            self.X_tranformed.unstack().to_frame().rename(columns={0: "durbin"}),
            left_on=["peak_label", "ms_file"],
            right_index=True,
        )
        ...
        self.df2["durbin_mh_score"] = (self.df2.durbin - self.df2.durbin_mh_mean) / (
            self.df2.durbin_mh_std
        )

        durbin = self.df2[
            self.df2.Type.isin(["Biological sample", self.media_type_name])
        ].sort_values(self.type_col_name, ascending=True)

        durbin["durbin_mh_score_masked"] = durbin["durbin_mh_score"]
        durbin["durbin_mh_score_low_projected_to_zero"] = durbin["durbin_mh_score"]
        durbin.loc[
            durbin["peak_max"] < self.low_values_mask,
            "durbin_mh_score_low_projected_to_zero",
        ] = 0
        durbin.loc[
            durbin["peak_max"] < self.low_values_mask, "durbin_mh_score_masked"
        ] = None
    ...
    def visualize(self, order=None, aspect=5, height=2.5, **kwargs):
        ...
        **kwargs : dict
        ...
        if order is None:
            order = (
                self.durbin[
                    (self.durbin[self.type_col_name] == "Biological sample")
                    & (self.durbin.durbin_mh_score_masked.notna())
                ]
                .groupby("peak_label")
                .durbin_mh_score_masked.median()
                .sort_values()
                .index.to_list()
            )
        ...
        g = sns.catplot(
            ...
            x="peak_label",
            y="durbin_mh_score",
            hue=self.type_col_name,
            dodge=False,
            hue_order=["Biological sample", self.media_type_name],
            order=order,
            aspect=aspect,
            height=height,
            marker=".",
