...
if __name__ == '__main__':
    ...
    parser.add_argument('--add-raw-name-to-outdir', default=True, action='store_false',
        help='Do not add subdirectory raw file name to run directory.')
    parser.add_argument('--add-uuid-to-rundir', default=True, action='store_false',
    ...
    for raw_file in args.raw[0]:
        if isfile(raw_file):
            mq.run(raw_file, cold_run=args.cold_run, rerun=args.rerun, submit=args.submit, run=True)
        else:
